...
def generate_basic_summary(findings):
    if not findings:
        return "No major profit leaks were detected in the uploaded data."

    total = sum(finding.dollar_impact for finding in findings)

    return (
        f"Profit Leak Hunter found {len(findings)} potential profit leak(s), "
        f"with an estimated total impact of ${abs(total):,.2f}."
    )
...
async def run_analysis(
    ...
    return AnalysisResponse(
        ...
        findings=findings,
        executive_summary=generate_basic_summary(findings),
        amd_usage_note=(
            "Current stage uses deterministic Python/pandas detectors. "
            "AMD/Fireworks LLM narrative generation will be added after core detection logic is stable."
        ),
    )
